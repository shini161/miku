...
export default new Command({
  ...
  run: async ({  message}) => {
    ...
    switch (target?.id) {
      case undefined:
        text = [`${message.author.username} wants to get booped.`];
      ...
      default:
        text = [
          `${message.author.username} is booping ${target.user.username}.`,
          `${message.author.username} boops ${target.user.username}.`,
        ];
        sendEmbed(text, images);
    }
